...
const InventoryPage = () => {
  ...
  return <div className="space-y-4 pb-20 sm:space-y-5">
    <header className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h2 className="flex items-center gap-3 text-xl font-bold tracking-tight text-slate-900 sm:text-2xl"><ModuleIcon icon={<FiBox />} module="inventory" variant="header" />Inventory & Recipes</h2><p className="mt-1 max-w-2xl text-sm text-slate-500">See stock health first, then manage adjustments and recipe ingredient consumption.</p></div><button type="button" onClick={load} disabled={loading} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-300 bg-white px-3 text-sm font-medium text-slate-700 shadow-sm disabled:opacity-60"><FiRefreshCw aria-hidden="true" />Refresh</button></header>
