...
import { FiAlertTriangle, FiCheckCircle, FiCpu, FiRefreshCw, FiSend } from "react-icons/fi";
import EmptyState from "../../components/common/EmptyState";
import { askIntelligence, getIntelligenceSummary, refreshIntelligence, updateIntelligenceInsight } from "../../services/intelligenceService";
...
const RestoSphereIntelligence = () => {
  ...
  const load = useCallback(async () => { setLoading(true); try { const response = await getIntelligenceSummary({ range }); setData(response.data?.data || null); } catch (error) { toast.error(error?.response?.data?.message || "Unable to load intelligence"); } finally { setLoading(false); } }, [range]);
  useEffect(() => { load(); }, [load]);
  const refresh = async () => { setRefreshing(true); try { const response = await refreshIntelligence({ range }); setData(response.data?.data || null); toast.success("Deterministic insights refreshed"); } catch (error) { toast.error(error?.response?.data?.message || "Unable to refresh insights"); } finally { setRefreshing(false); } };
  const ask = async (event) => { event.preventDefault(); if (!question.trim()) return; try { const response = await askIntelligence({ question: question.trim(), range }); setAnswer(response.data?.data || null); } catch (error) { toast.error(error?.response?.data?.message || "Unable to answer this analytics question"); } };
  const acknowledge = async (insight) => { if (!insight?._id) return; try { await updateIntelligenceInsight(insight._id, "ACKNOWLEDGED"); toast.success("Insight acknowledged"); await load(); } catch (error) { toast.error(error?.response?.data?.message || "Unable to update insight"); } };
...
export default RestoSphereIntelligence;
