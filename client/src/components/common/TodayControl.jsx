...
const TodayControl = ({ className = "", detailed = false }) => {
  ...
  return <div aria-label={`Current reporting period: Today, ${dateLabel}`} title={`Current reporting period: Today, ${dateLabel}`} className={`flex min-h-11 min-w-0 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50/70 px-2.5 py-1.5 text-sm font-medium text-slate-700 shadow-sm transition hover:border-slate-300 ${className}`}>
    <FiCalendar className="h-4 w-4 shrink-0 text-emerald-700" />
