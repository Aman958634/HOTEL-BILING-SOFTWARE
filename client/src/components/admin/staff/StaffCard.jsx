  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button onClick={() => onView(staff)} className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm text-slate-700"><span className="inline-flex items-center gap-1"><FiEye /> View</span></button>
        <button onClick={() => onEdit(staff)} className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm text-slate-700"><span className="inline-flex items-center gap-1"><FiEdit2 /> Edit</span></button>
        <button onClick={() => onStatus(staff)} className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm text-slate-700">{String(staff.status || "ACTIVE").toUpperCase() === "ACTIVE" ? "Deactivate" : "Reactivate"}</button>
        {canDelete && <button onClick={() => onDelete(staff)} className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-1.5 text-sm text-rose-700"><span className="inline-flex items-center gap-1"><FiTrash2 /> Delete</span></button>}
      </div>
    </article>
  );
};
