...
const StatCard = ({ icon, iconModule = "dashboard", iconTone, iconVariant = "kpi", label, value, trend = 0, formatValue, range = "today", comparisonType = "dashboard", compact = false, showComparison = true }) => {
  ...
  return (
    <div className={`min-w-0 rounded-2xl border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md ${compact ? "p-3 sm:p-4" : "p-5"}`}>
      <div className="flex min-w-0 items-start justify-between gap-2">
        <ModuleIcon icon={icon} module={iconModule} tone={iconTone} variant={iconVariant} />
