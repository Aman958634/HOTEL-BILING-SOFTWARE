...
const OutletSwitcher = () => {
  ...
  return <label className="flex min-w-0 items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-2 py-1.5 text-sm text-slate-700"><FiMapPin className="shrink-0 text-emerald-700" /><span className="sr-only">Active outlet</span><select value={activeOutletId} onChange={changeOutlet} className="max-w-36 bg-transparent text-sm font-medium outline-none sm:max-w-48">{outlets.map((outlet) => <option key={outlet._id} value={outlet._id}>{outlet.name}</option>)}</select></label>;
};
