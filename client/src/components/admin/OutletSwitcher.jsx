...
const OutletSwitcher = ({ className = "" }) => {
  ...
  return <label className={`flex min-h-11 min-w-0 max-w-full items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-2 py-1.5 text-sm text-slate-700 shadow-sm ${className}`}><FiMapPin className="shrink-0 text-emerald-700" aria-hidden="true" /><span className="sr-only">Active outlet</span><select aria-label="Select active outlet" value={activeOutletId} onChange={changeOutlet} className="min-w-0 max-w-36 flex-1 truncate bg-transparent text-sm font-medium outline-none focus-visible:ring-2 focus-visible:ring-brand-600 sm:max-w-48">{outlets.map((outlet) => <option key={outlet._id} value={outlet._id}>{outlet.name}</option>)}</select></label>;
};
