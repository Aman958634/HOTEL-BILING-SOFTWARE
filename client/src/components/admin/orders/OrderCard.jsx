...
const OrderCard = ({ order, onEdit, onDelete }) => (
  <article className="min-w-0 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
    ...
    <dl className="mt-3 grid grid-cols-2 gap-2 rounded-xl bg-slate-50 p-2.5 text-sm">
      ...
      <div className="min-w-0">
        <dt className="text-[11px] font-medium uppercase tracking-wide text-slate-500">Payment</dt>
        <dd className="mt-1"><span className={`inline-flex max-w-full items-center rounded-full border px-2 py-0.5 text-xs font-medium ${paymentBadgeClasses(order.paymentStatus)}`}>{paymentStatusLabel(order.paymentStatus)}</span></dd>
      </div>
    </dl>

    <p className="mt-2 break-words text-xs text-slate-500">{order.items?.length || 0} items <span aria-hidden="true">·</span> {paymentMethodText(order.paymentMethod)}</p>

    <div className="mt-4 flex gap-2">
      <button
        type="button"
        onClick={() => onEdit(order)}
        className="inline-flex min-h-11 flex-1 items-center justify-center gap-1.5 rounded-xl border border-teal-200 bg-white px-3 text-sm font-medium text-teal-700 transition hover:bg-teal-50 focus:outline-none focus:ring-2 focus:ring-teal-500/30"
      ...
      <button
        ...
        className="inline-flex min-h-11 flex-1 items-center justify-center gap-1.5 rounded-xl border border-rose-200 bg-white px-3 text-sm font-medium text-rose-700 transition hover:bg-rose-50 focus:outline-none focus:ring-2 focus:ring-rose-500/30"
