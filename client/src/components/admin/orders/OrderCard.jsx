...
const OrderCard = ({ order, onOpen, onEdit, onDelete, kitchenOnly = false }) => (
  <article className="min-w-0 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:p-4">
    ...
    {kitchenOnly ? <div className="mt-3 rounded-xl bg-slate-50 p-2.5 text-sm"><p className="font-medium text-slate-700">Kitchen items</p><ul className="mt-1 space-y-1 text-slate-600">{order.items?.map((item) => <li key={item._id || item.name}>{item.quantity} x {item.name}</li>)}</ul>{order.notes || order.specialInstructions ? <p className="mt-2 text-xs text-slate-500">{order.notes || order.specialInstructions}</p> : null}</div> : <dl className="mt-3 grid grid-cols-2 gap-2 rounded-xl bg-slate-50 p-2.5 text-sm">
      <div className="min-w-0">
