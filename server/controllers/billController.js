...
import { emitPaymentCreated } from "../socket/paymentSocket.js";
...
export const listEligibleOrders = asyncHandler(async (req, res) => {
  const filter = await buildOutletQuery({ isArchived: { $ne: true }, status: { $nin: ["CANCELLED", "REJECTED"] }, billingBill: null }, req.user);
...
export const createBill = asyncHandler(async (req, res) => {
  const restaurantFilter = await buildOutletQuery({}, req.user); const restaurantId = restaurantFilter.restaurant;
...
export const listBills = asyncHandler(async (req, res) => {
  const { page, limit, skip } = pagination(req.query); const filter = await buildOutletQuery({}, req.user);
...
export const addBillPayment = asyncHandler(async (req, res) => {
  const filter = await buildOutletQuery({}, req.user); const restaurantId = filter.restaurant;
  ...
  if (!result.idempotent) {
    emitPaymentCreated(result.payment.toObject ? result.payment.toObject() : result.payment);
...
export const cancelBill = asyncHandler(async (req, res) => {
  const bill = await Bill.findOne(await buildOutletQuery({ _id: req.params.id }, req.user)); if (!bill) throw new ApiError(404, "Bill not found");
