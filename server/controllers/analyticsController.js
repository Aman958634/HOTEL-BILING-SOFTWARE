...
export const dashboardStats = asyncHandler(async (req, res) => {
  ...
  const [metrics, foods, inventory, series, orderSeries] = await Promise.all([
    ...
    getSettledOrderCountSeries({ scope: inventoryScope, range: chartRange }),
  ]);
  const dailySales = series.slice(-14).map((row) => {
    const orderRow = orderSeries.find((o) => o.label === row.label);
