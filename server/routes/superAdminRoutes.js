...
router.post("/subscriptions/:id/convert", [...subscriptionIdValidation, body("planName").optional().isString().trim().isLength({ min: 1, max: 120 }).withMessage("Plan selection is invalid"), body("planId").optional().isMongoId().withMessage("Plan selection is invalid"), body("premiumDurationYears").optional().isInt({ min: 1, max: 5 }).withMessage("Premium duration must be between 1 and 5 years")], validate, subscriptionsCtrl.convertToPaid);
