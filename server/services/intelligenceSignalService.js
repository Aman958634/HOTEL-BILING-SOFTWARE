...
// Insight evidence is a display-ready contract: the label is always text and
// comparison values are always scalar values. Never pass a BI metric object
// through as `metric`, because the client renders that field as a text label.
const evidence = (label, metric, unit = "number") => ({ metric: label, current: number(metric?.current), baseline: number(metric?.previous), change: metric?.growth?.value ?? null, unit });
...
export const detectSignals = (bi, thresholds = DEFAULT_INTELLIGENCE_THRESHOLDS) => {
  ...
  if (orders < thresholds.minimumOrdersForTrend || priorOrders < thresholds.minimumOrdersForTrend) {
    signals.push(insight({ signalKey: "insufficient-history", category: "DATA_QUALITY", severity: "INFO", title: "Not enough comparable order history", summary: `Only ${orders} qualifying order(s) are available in this period; trend alerts are held back until both periods have enough activity.`, evidence: [evidence("Qualifying orders", bi.overview?.orders, "orders")], actions: ["Continue recording completed orders to strengthen comparisons."], confidence: "LOW", bi }));
  } else {
    if (salesChange !== null && salesChange <= -thresholds.salesChangePercent) signals.push(insight({ signalKey: "net-sales-decline", category: "SALES", severity: "ATTENTION", title: "Net sales decreased", summary: `Net sales are ${Math.abs(salesChange)}% below the previous comparable period (${number(bi.overview.netSales.current)} versus ${number(bi.overview.netSales.previous)}).`, evidence: [evidence("Net sales", bi.overview.netSales, "INR"), evidence("Qualifying orders", bi.overview.orders, "orders")], actions: ["Review order volume by source and peak-hour traffic before changing prices or menu items."], confidence: "HIGH", bi }));
    if (salesChange !== null && salesChange >= thresholds.salesChangePercent) signals.push(insight({ signalKey: "net-sales-increase", category: "SALES", severity: "OPPORTUNITY", title: "Net sales increased", summary: `Net sales are ${salesChange}% above the previous comparable period.`, evidence: [evidence("Net sales", bi.overview.netSales, "INR")], actions: ["Review the strongest order sources and service periods to understand the increase."], confidence: "HIGH", bi }));
    if (orderChange !== null && orderChange <= -thresholds.orderChangePercent) signals.push(insight({ signalKey: "order-volume-decline", category: "ORDERS", severity: "ATTENTION", title: "Order volume declined", summary: `Qualifying order volume is ${Math.abs(orderChange)}% below the previous comparable period.`, evidence: [evidence("Qualifying orders", bi.overview.orders, "orders")], actions: ["Review availability, order channels, and service demand for the selected period."], confidence: "HIGH", bi }));
  }
...
export const answerAllowedQuestion = ({ question, snapshot }) => {
  ...
  if (/(sales|revenue|week|today)/.test(input)) return { intent: "SALES_SUMMARY", answer: `Net sales are ${number(bi.overview.netSales.current)} for the selected period, compared with ${number(bi.overview.netSales.previous)} previously.`, evidence: [evidence("Net sales", bi.overview.netSales, "INR"), evidence("Qualifying orders", bi.overview.orders, "orders")] };
  ...
  if (/(refund|payment|cash|reconcil)/.test(input)) return { intent: "PAYMENT_MIX", answer: `Net collected is ${number(bi.overview.netCollected.current)}. There are ${number(bi.payments.reconciliation?.unreconciledPayments)} unreconciled payment record(s).`, evidence: [evidence("Net collected", bi.overview.netCollected, "INR"), { metric: "Unreconciled payments", current: number(bi.payments.reconciliation?.unreconciledPayments), unit: "payments" }] };
