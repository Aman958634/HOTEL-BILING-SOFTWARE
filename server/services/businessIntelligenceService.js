...
const qualifyingOrderMatch = (restaurant, range) => ({ restaurant, isArchived: { $ne: true }, status: { $in: completedStatuses }, ...dateMatch("createdAt", range) });
const successfulPaymentMatch = (restaurant, range) => ({ restaurant, paymentStatus: { $in: paidStatuses }, $or: [{ paidAt: { $gte: range.start, $lt: range.end } }, { paidAt: null, createdAt: { $gte: range.start, $lt: range.end } }] });
...
const salesForRange = async (restaurant, range) => {
  const match = qualifyingOrderMatch(restaurant, range);
...
const paymentsForRange = async (restaurant, range) => {
  const match = successfulPaymentMatch(restaurant, range);
  const [totals, methods, refundRows, reconciliationRows] = await Promise.all([
    Promise.all([Payment.aggregate([{ $match: match }, { $group: { _id: null, received: { $sum: "$amount" }, transactions: { $sum: 1 } } }]), Payment.aggregate([{ $match: { restaurant, refundAmount: { $gt: 0 }, refundedAt: { $gte: range.start, $lt: range.end } } }, { $group: { _id: null, refunded: { $sum: "$refundAmount" } } }])]),
    Payment.aggregate([{ $match: match }, { $group: { _id: "$paymentMethod", amount: { $sum: "$amount" }, transactions: { $sum: 1 } } }, { $sort: { amount: -1 } }]),
    Payment.aggregate([{ $match: { restaurant, refundAmount: { $gt: 0 }, refundedAt: { $gte: range.start, $lt: range.end } } }, { $group: { _id: "$refundStatus", amount: { $sum: "$refundAmount" }, count: { $sum: 1 } } }]),
    Promise.all([Payment.countDocuments({ restaurant, reconciliationStatus: { $in: ["UNRECONCILED", "MISMATCHED", "UNDERPAID", "OVERPAID"] } }), CashReconciliation.aggregate([{ $match: { restaurant, closedAt: { $gte: range.start, $lt: range.end }, status: "MISMATCHED" } }, { $group: { _id: null, variance: { $sum: "$difference" }, count: { $sum: 1 } } }]), Refund.countDocuments({ restaurant, status: "PENDING" })]),
  ]);
...
const menuForRange = async (restaurant, range) => {
  const match = qualifyingOrderMatch(restaurant, range);
...
const customersForRange = async (restaurant, range) => {
  const match = { ...qualifyingOrderMatch(restaurant, range), customer: { $ne: null } };
  const [customers, firstOrders] = await Promise.all([Order.aggregate([{ $match: match }, { $group: { _id: "$customer", orders: { $sum: 1 }, spend: { $sum: "$total" } } }]), Order.aggregate([{ $match: { restaurant, isArchived: { $ne: true }, status: { $in: completedStatuses }, customer: { $ne: null } } }, { $group: { _id: "$customer", firstOrderAt: { $min: "$createdAt" } } }])]);
...
const operationsForRange = async (restaurant, range) => {
  const match = qualifyingOrderMatch(restaurant, range); const [tables, kots, staff, inventory, movements] = await Promise.all([
    Order.aggregate([{ $match: { ...match, table: { $ne: null } } }, { $group: { _id: "$table", orders: { $sum: 1 }, sales: { $sum: "$total" } } }, { $sort: { sales: -1 } }, { $limit: 8 }]),
    KotTicket.aggregate([{ $match: { restaurant, createdAt: { $gte: range.start, $lt: range.end } } }, { $group: { _id: "$status", count: { $sum: 1 } } }]),
    ...
    Inventory.aggregate([{ $match: { restaurant, isActive: true } }, { $group: { _id: null, stockValue: { $sum: { $multiply: ["$quantity", "$costPerUnit"] } }, lowStock: { $sum: { $cond: [{ $lte: ["$quantity", "$reorderLevel"] }, 1, 0] } } } }]),
    StockMovement.aggregate([{ $match: { restaurant, movementType: "WASTAGE", createdAt: { $gte: range.start, $lt: range.end } } }, { $group: { _id: null, quantity: { $sum: "$quantity" }, count: { $sum: 1 } } }]),
...
export const getBusinessIntelligenceOverview = async ({ restaurantId, query }) => {
  const restaurant = await Restaurant.findById(restaurantId).select("timeZone").lean(); const timeZone = restaurant?.timeZone || "Asia/Kolkata"; const currentRange = resolveBusinessRange({ ...query, timeZone }); const previousRange = { ...currentRange, start: currentRange.previousStart, end: currentRange.previousEnd };
  const [sales, payments, menu, customers, operations, previousSales, previousPayments] = await Promise.all([salesForRange(restaurantId, currentRange), paymentsForRange(restaurantId, currentRange), menuForRange(restaurantId, currentRange), customersForRange(restaurantId, currentRange), operationsForRange(restaurantId, currentRange), salesForRange(restaurantId, previousRange), paymentsForRange(restaurantId, previousRange)]);
